...
app = typer.Typer(
    name="interactive",
    help="🎮 Commandes interactives modernes",
...
def _quick_start_wizard():
    ...
    enable_autoscale = questionary.confirm(
        "Activer l'auto-scaling des agents?",
        default=True,
        style=custom_style
    ).ask()
...
def _data_analysis_wizard():
    ...
    console.print(f"[bold #06FFA5]✅ Analyse terminée! Rapport: analysis_report.{output_format.lower()}[/bold #06FFA5]")
...
def _training_wizard():
    ...
    while True:
        ...
        if batch_int not in VALID_BATCH_SIZES:
            console.print(f"[bold yellow]⚠️  Batch size {batch_int} n'est pas optimal. Valeurs recommandées: {VALID_BATCH_SIZES}[/bold yellow]")
        break

    # Learning rate avec validation de format
    while True:
        learning_rate = questionary.text(
            "Learning rate (ex: 2e-4, 0.0001):",
            default="2e-4",
            style=custom_style
        ).ask()
    ...
    if questionary.confirm(
        ...
        console.print("[bold #B0B0B0]Utilisez 'tawiza finetune status' pour suivre la progression[/bold #B0B0B0]")
