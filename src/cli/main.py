...
app.add_typer(data_group, name="data", help="Gestion des Donnees (import, annotate)")
...
# Commandes directes (retrocompatibilite et acces rapide)
app.add_typer(agents_app, name="agents", help="Agents IA (raccourci)")
app.add_typer(debug_app, name="debug", help="Debogage et monitoring")
...
app.add_typer(learning_app, name="learning", help="Learning Pipeline (Label Studio + LLaMA-Factory)")
...
try:
    ...
    app.add_typer(mascot_demo_app, name="mascot-demo", help="🐱 Démonstration des animations mascotte")
except ImportError:
    pass  # Module optionnel
...
def quickstart(
    ...
    interactive: bool = typer.Option(True, "--interactive/--no-interactive", help="Mode interactif")
):
    """Démarrage rapide avec configuration optimale"""

    show_sunset_header()

    console.print(f"[bold {THEME_DICT['info_color']}]{Emoji.ROCKET} Démarrage rapide de {APP_NAME}...[/bold {THEME_DICT['info_color']}]")
    console.print()

    try:
        # Étape 1: Vérifier la configuration système
        console.print(f"[bold {THEME_DICT['accent_color']}]📋 Étape 1: Vérification du système...[/bold {THEME_DICT['accent_color']}]")
        ...
        console.print(f"[bold {THEME_DICT['accent_color']}]⚡ Étape 2: Initialisation du système...[/bold {THEME_DICT['accent_color']}]")
        ...
        if monitoring:
            console.print(f"[bold {THEME_DICT['accent_color']}]🔍 Étape 3: Démarrage du monitoring...[/bold {THEME_DICT['accent_color']}]")
        ...
        console.print(f"[bold {THEME_DICT['accent_color']}]🎯 Étape 4: Commandes de démarrage...[/bold {THEME_DICT['accent_color']}]")
        console.print()

        console.print(f"[bold {THEME_DICT['info_color']}]Votre système est prêt! Voici quelques commandes pour commencer:[/bold {THEME_DICT['info_color']}]")
        console.print()

        if gpu:
            console.print(f"  🎮 {THEME_DICT['accent_color']}tawiza agents gpu-optimize --model qwen3.5:27b --benchmark[/]{THEME_DICT['text_color']}")
            console.print("     Optimiser les performances GPU")
            console.print()

        console.print(f"  📊 {THEME_DICT['accent_color']}tawiza agents analyze-data data/dataset.csv[/]{THEME_DICT['text_color']}")
        console.print("     Analyser un dataset")
        console.print()

        console.print(f"  💻 {THEME_DICT['accent_color']}tawiza agents generate-code 'Créer une API REST' --language python --framework fastapi[/]{THEME_DICT['text_color']}")
        console.print("     Générer du code")
        console.print()

        if monitoring:
            console.print(f"  🐛 {THEME_DICT['accent_color']}tawiza debug monitor --realtime --duration 60[/]{THEME_DICT['text_color']}")
            console.print("     Monitorer en temps réel")
            console.print()

        console.print(f"  📚 {THEME_DICT['accent_color']}tawiza --help[/]{THEME_DICT['text_color']}")
        console.print("     Voir toutes les commandes disponibles")
        console.print()

        if interactive:
            console.print(f"[bold {THEME_DICT['success_color']}]✅ Démarrage rapide terminé avec succès![/bold {THEME_DICT['success_color']}]")
            console.print()
            console.print(f"[bold {THEME_DICT['info_color']}]💡 Astuce: Utilisez 'tawiza quickstart --interactive' pour un guide guidé[/bold {THEME_DICT['info_color']}]")
        else:
            console.print(f"[bold {THEME_DICT['success_color']}]✅ Démarrage rapide terminé![/bold {THEME_DICT['success_color']}]")

    except Exception as e:
        console.print(f"\n[bold {THEME_DICT['error_color']}]❌ Erreur lors du démarrage rapide: {e}[/bold {THEME_DICT['error_color']}]")
        console.print(f"[bold {THEME_DICT['info_color']}]💡 Utilisez 'tawiza system status' pour diagnostiquer le problème[/bold {THEME_DICT['info_color']}]")
        raise typer.Exit(1)
...
def demo(
    ...
    console.print(f"[bold {THEME_DICT['info_color']}]🎮 Démonstration interactive de Tawiza-V2[/bold {THEME_DICT['info_color']}]")
    console.print()

    try:

        # Vérifier que le système est initialisé
        console.print(f"[bold {THEME_DICT['accent_color']}]📋 Vérification du système...[/bold {THEME_DICT['accent_color']}]")
        ...
        if not auto:
            # Mode interactif
            console.print(f"[bold {THEME_DICT['info_color']}]🎯 Démonstration interactive[/bold {THEME_DICT['info_color']}")
            console.print()
...
def demo_data_analysis(duration: int, auto: bool):
    ...
    try:
        console.print(f"  • Dataset créé: {len(demo_data)} colonnes, {len(demo_data['temperature'])} lignes")

        # Appeler la commande d'analyse
        from src.cli.commands.agents_advanced import analyze_data
        analyze_data(
            dataset=dataset_path,
            output="demo_analysis.json",
            detailed=True,
            wait=True
        )
...
def tui():
    ...
    console.print(f"[bold {THEME_DICT['info_color']}]🖥️  Lancement de l'interface TUI...[/bold {THEME_DICT['info_color']}]")
    console.print()
    ...
    try:
        ...
        console.print(f"[bold {THEME_DICT['error_color']}]❌ TUI dependencies not installed[/bold {THEME_DICT['error_color']}]")
        console.print(f"[bold {THEME_DICT['info_color']}]→ Run: pip install textual psutil[/bold {THEME_DICT['info_color']}]")
        raise typer.Exit(1)
    except Exception as e:
        console.print(f"[bold {THEME_DICT['error_color']}]❌ Erreur lors du lancement du TUI: {e}[/bold {THEME_DICT['error_color']}]")
        raise typer.Exit(1)
...
def wizard():
    ...
    console.print(f"[bold {THEME_DICT['info_color']}]🧙 Assistant de configuration Tawiza-V2[/bold {THEME_DICT['info_color']}]")
    console.print()

    try:
        # Étape 1: Présentation
        console.print(f"[bold {THEME_DICT['accent_color']}]Bienvenue dans l'assistant Tawiza-V2![/bold {THEME_DICT['accent_color']}]")
        console.print()
        ...
        if gpu_enabled:
            console.print(f"  🎮 {THEME_DICT['accent_color']}tawiza agents gpu-optimize --model qwen3.5:27b[/]{THEME_DICT['text_color']}")
            console.print("     Optimiser les performances GPU")
            console.print()
        ...
        console.print(f"[bold {THEME_DICT['success_color']}]✅ Configuration terminée avec succès![/bold {THEME_DICT['success_color']}]")
        console.print()
        console.print(f"[bold {THEME_DICT['info_color']}]💡 Vous pouvez maintenant utiliser toutes les commandes Tawiza-V2![/bold {THEME_DICT['info_color']}]")

    except Exception as e:
        console.print(f"\n[bold {THEME_DICT['error_color']}]❌ Erreur dans l'assistant: {e}[/bold {THEME_DICT['error_color']}]")
        console.print(f"[bold {THEME_DICT['info_color']}]💡 Réessayez ou utilisez 'tawiza system status' pour diagnostiquer[/bold {THEME_DICT['info_color']}]")
        raise typer.Exit(1)
...
def show_version():
    ...
    try:
        ...
        version_text.append("🐛 Système de Débogage Intelligent\n", style=SUNSET_THEME["info_color"])
        ...
        console.print(f"[bold {THEME_DICT['error_color']}]❌ Impossible de déterminer la version[/bold {THEME_DICT['error_color']}]")

@app.command("mascot")
def show_mascot_cmd(
    style: str = typer.Option("welcome", "--style", "-s",
        help="Style de mascotte: welcome, banner, coder, hacker, realistic, laptop"),
    mood: str = typer.Option(None, "--mood", "-m",
        help="Humeur: default, happy, thinking, working, success, error"),
