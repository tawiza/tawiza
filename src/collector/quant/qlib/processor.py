...
class FillnaProcessor(Processor):
    ...
    def __init__(self, fill_value: Union[float, int] = 0, 
                 method: str = 'value', 
        ...
        for field in fields_to_process:
            if self.method == 'global':
                ...
                if isinstance(data.index, pd.MultiIndex):
                    ...
                    result[field] = data[field].rolling(self.window, min_periods=1).apply(calc_robust_zscore)
...
class PopulationNormalizer(Processor):
    ...
    def __init__(self, population_data: Union[pd.DataFrame, dict],
                 per_n: int = 10000,
                 fields: Optional[List[str]] = None):
        ...
        if isinstance(population_data, pd.DataFrame):
            if 'code_dept' in population_data.columns and 'population' in population_data.columns:
                self.pop_dict = dict(zip(
                    population_data['code_dept'], 
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.fields is not None:
            ...
            count_fields = [
                'liquidations', 'liquidation_judiciaire', 'creation_entreprise',
                'fermeture_entreprise', 'offres_emploi', 'logements_autorises',
                'transactions_immobilieres', 'vente_fonds_commerce'
            ]
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(data.index, pd.MultiIndex):
            ...
            for field in fields_to_process:
                ...
                result[f"{field}_deseasonalized"] = data.groupby(level=1)[field].apply(deseasonalize)
...
def create_standard_chain(population_data: Optional[Union[pd.DataFrame, dict]] = None,
                         robust: bool = True) -> ProcessorChain:
    """
...
def create_inference_chain(population_data: Optional[Union[pd.DataFrame, dict]] = None) -> ProcessorChain:
    """
    ...
    return ProcessorChain([
        ...
        PopulationNormalizer(population_data) if population_data is not None else FillnaProcessor(fill_value=0),
