...
class ShortTermMemory:
    ...
    def add_turn(self, turn: ConversationTurn):
        ...
        self.recent_turns = [
            turn for turn in self.recent_turns
    ...
    def get_context_summary(self) -> str:
        ...
        for turn in self.recent_turns[-3:]:  # 3 derniers tours
            if turn.role == "user":
                ...
                key_points.append(turn.content[:100] + "..." if len(turn.content) > 100 else turn.content)
...
class LongTermMemory:
    ...
    def _init_vector_search(self) -> None:
        ...
        try:
            ...
            embeddings_config = EmbeddingsConfig(
                model=self._qdrant_config["embedding_model"]
            )
    ...
    async def _generate_conversation_embedding(self, turns: list[ConversationTurn]) -> list[float] | None:
    ...
    def store_conversation(self, context: ConversationContext, turns: list[ConversationTurn]):
        ...
        try:
            ...
            ''', (
                context.session_id, context.session_id, context.user_id,
                context.agent_type, context.start_time, context.last_update,
                context.summary, json.dumps(context.key_topics),
                json.dumps(context.user_preferences), context.conversation_length
            ))
            ...
            for turn in turns:
                ...
                ''', (
                    turn.id, context.session_id, turn.timestamp, turn.role,
                    turn.content, json.dumps(turn.metadata),
    ...
    def retrieve_similar_conversations(self, query: str, limit: int = 5) -> list[tuple[ConversationContext, list[ConversationTurn]]]:
...
class ContextManager:
    ...
    def get_personalized_response(self, user_input: str, agent_context: dict[str, Any]) -> dict[str, Any]:
...
class PersonalizationEngine:
    ...
    def learn_from_interaction(self, user_id: str, interaction_data: dict[str, Any]):
        ...
        if "preferred_model_type" in interaction_data:
            profile["preferences"]["preferred_model_type"] = interaction_data["preferred_model_type"]
        ...
        if "success" in interaction_data:
            ...
            # Moyenne mobile
            profile["success_rate"] = (current_success * (profile["interaction_count"] - 1) + new_success) / profile["interaction_count"]

        logger.info(f"🎯 Apprentissage pour utilisateur {user_id}: {len(interaction_data)} patterns mis à jour")
    ...
    def predict_user_preference(self, user_id: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if profile["success_rate"] > 0.8:
            ...
            predictions["suggested_actions"] = ["Utiliser l'approche précédente", "Activer l'analyse détaillée"]
            predictions["preferred_approach"] = "detailed"
        elif profile["success_rate"] > 0.6:
            predictions["confidence"] = 0.7
            predictions["suggested_actions"] = ["Suivre les recommandations", "Vérifier les paramètres"]
...
class AdvancedConversationMemory:
    ...
    def process_interaction(self, user_input: str, current_context: ConversationContext) -> dict[str, Any]:
