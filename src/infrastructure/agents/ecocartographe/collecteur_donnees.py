    ...
    "annuaires_nationaux": [
        SourceWeb(
            ...
            selecteurs={
                "liste": ".startup-card",
                "nom": ".startup-name",
                "ville": ".startup-location",
            },
        ),
        SourceWeb(nom="BPI France", url="https://www.bpifrance.fr/", type_source="institutionnel"),
    ],
    "clusters": [
        SourceWeb(
            nom="France Clusters", url="https://franceclusters.fr/annuaire/", type_source="cluster"
        ),
    ],
    "poles_competitivite": [
        SourceWeb(
...
class CollecteurDonnees:
    ...
    async def _rechercher_api_entreprises(
        self, territoire: str, thematique: str | None, limite: int
    ) -> list[Acteur]:
    ...
    def _entreprise_api_vers_acteur(self, data: dict[str, Any]) -> Acteur | None:
        ...
        try:
            ...
            if siege:
                ...
                if siege.get("latitude") and siege.get("longitude"):
                    adresse.coordonnees = Coordonnees(
                        latitude=float(siege["latitude"]), longitude=float(siege["longitude"])
                    )
            ...
            acteur = Acteur(
                ...
                secteurs=[data.get("libelle_activite_principale", "")]
    ...
    def _determiner_type_depuis_api(
        self, nature_juridique: str, categorie: str, activite: str, nom: str
    ) -> ActeurType:
        ...
        if any(
            kw in nom_lower for kw in ["université", "univ ", "ufr", "iut", "école", "institut"]
        ):
            return ActeurType.UNIVERSITE
        elif any(
            kw in nom_lower
            for kw in ["laboratoire", "lab ", "cnrs", "inria", "cea", "inserm", "inrae"]
        ):
    ...
    async def _extraire_noms_entreprises(self, texte: str, territoire: str) -> list[str]:
        ...
        for pattern in patterns:
            ...
            for match in matches:
                nom = match.strip()
                if len(nom) >= 3 and nom.lower() not in [
                    "sas",
                    "sarl",
                    "france",
                    territoire.lower(),
...
Liste {min(limite, 20)} acteurs réels et vérifiables."""
                        ...
                        for item in data:
                            if item.get("nom") and item["nom"].lower() not in self._acteurs_vus:
                                acteur = Acteur(
                                    ...
                                    adresse=Adresse(ville=item.get("ville"))
                                    if item.get("ville")
                                    else None,
    ...
    async def _geocoder_adresse(self, adresse: Adresse) -> Coordonnees | None:
        ...
        try:
            adresse_str = " ".join(
                filter(None, [adresse.rue, adresse.code_postal, adresse.ville, "France"])
            )
            ...
            if response.status_code == 200:
                ...
                if results:
                    return Coordonnees(
                        latitude=float(results[0]["lat"]), longitude=float(results[0]["lon"])
                    )
        except Exception as e:
    ...
    async def sauvegarder_dataset(
        self, resultat: ResultatRecherche, nom_fichier: str, format: str = "json"
    ) -> str:
