...
class CognitiveEngine:
    ...
    async def process(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        logger.info(
            f"Data quality assessment: {data_quality['data_count']} items, sources={data_quality['sources']}, quality={data_quality['quality_score']:.2f}"
        )
        ...
        for level_num, (name, level) in enumerate(self.levels, 1):
            ...
            if (
                name in ("scenario", "strategy", "theoretical")
                and not data_quality["has_real_data"]
            ):
                logger.warning(
                    f"Skipping {name} level: insufficient real data (count={data_quality['data_count']})"
                )
                output = {
                    "skipped": True,
            ...
            if name == "discovery":
                ...
                if not signals and not data_quality["has_real_data"]:
                    logger.warning(
                        "Discovery found no signals and no real data - flagging subsequent levels"
                    )

        # Aggregate confidence (weighted average, higher levels weighted more)
        weights = [1, 2, 3, 4, 5]
        avg_confidence = sum(c * w for c, w in zip(confidences, weights, strict=False)) / sum(
            weights
        )
    ...
    async def process_unified(
        ...
        data_quality = self._assess_data_quality(results)
        logger.info(
            f"[COMPLETE mode] Data quality: {data_quality['data_count']} items from {data_quality['sources']}"
        )
        ...
        if tier_override and self._llm_provider:
            ...
            logger.info(
                f"[COMPLETE mode] Setting tier_override={tier_override} for all cognitive levels"
            )
    ...
    async def process_level(
        self, level_name: str, results: list[dict[str, Any]], previous: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
