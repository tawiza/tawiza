...
class AdvancedDebugger:
    ...
    def _get_gpu_summary(self, metrics: list[PerformanceMetrics]) -> dict[str, Any]:
        ...
        return {
            ...
            "memory": {
                "average": sum(gpu_memory_values) / len(gpu_memory_values) if gpu_memory_values else 0,
    ...
    def _get_agent_status_summary(self) -> dict[str, Any]:
        ...
        active_agents = sum(1 for agent in self.agent_debug_info.values() if agent.status == "active")
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if self.error_tracker:
            total_errors = sum(self.error_tracker.values())
            if total_errors > 10:
                recommendations.append("Taux d'erreur élevé - analysez les logs pour identifier les problèmes")

        # Analyser les agents
        if self.agent_debug_info:
            for agent_id, agent in self.agent_debug_info.items():
                if agent.error_count > 5:
                    recommendations.append(f"Agent {agent_id} a des problèmes - redémarrez ou vérifiez la configuration")
        ...
        if self.memory_snapshots:
            ...
            if len(self.memory_snapshots) >= 5:
                ...
                for i in range(1, min(5, len(self.memory_snapshots))):
                    growth = self.memory_snapshots[i].total_memory - self.memory_snapshots[i-1].total_memory
                    recent_growth.append(growth)

                avg_growth = sum(recent_growth) / len(recent_growth)
                if avg_growth > 10 * 1024 * 1024:  # 10MB average growth
                    recommendations.append("Croissance mémoire détectée - vérifiez les fuites mémoire")
...
def create_advanced_debugger(debug_level: str = "INFO", enable_profiling: bool = True) -> AdvancedDebugger:
