...
def create_access_token(subject: str, scopes: list[str] | None = None, expires_delta: timedelta | None = None) -> str:
...
def verify_token(token: str) -> TokenData:
    ...
    # DEV BYPASS
    if token == "dev-token":
        return TokenData(sub="00000000-0000-0000-0000-000000000000", exp=datetime.now(UTC) + timedelta(days=1), iat=datetime.now(UTC), scopes=["admin"])
...
async def get_current_user(credentials: HTTPAuthorizationCredentials | None = Depends(security_scheme)) -> User:
