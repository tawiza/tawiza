...
class TaskHandler:
    ...
    def _register_handlers(self):
        ...
        self.manager.register_handler(MessageType.TASK_CREATE, self.handle_create)
        self.manager.register_handler(MessageType.TASK_PAUSE, self.handle_pause)
        self.manager.register_handler(MessageType.TASK_RESUME, self.handle_resume)
        self.manager.register_handler(MessageType.TASK_CANCEL, self.handle_cancel)
        self.manager.register_handler(MessageType.TASK_CORRECT, self.handle_correct)

    async def handle_create(self, message: TaskCreateMessage, websocket: WebSocket) -> None:
    ...
    async def _execute_task(self, task_id: str) -> None:
        ...
        try:
            ...
            await self.manager.broadcast(TaskErrorMessage(task_id=task_id, error="Task cancelled"))
        except Exception as e:
            task["status"] = TaskStatus.FAILED
            logger.error(f"Task {task_id} failed: {e}")
            await self.manager.broadcast(TaskErrorMessage(task_id=task_id, error=str(e)))
        finally:
    ...
    async def _handle_task_event(self, task_id: str, event: TaskEvent) -> None:
        ...
        if event_type == TaskEventType.STARTED:
            ...
            await self.manager.broadcast(
                TaskThinkingMessage(
                    task_id=task_id, content=f"Starting {event.data.get('agent', 'agent')}..."
                )
            )

        elif event_type == TaskEventType.THINKING:
            await self.manager.broadcast(
                TaskThinkingMessage(task_id=task_id, content=event.data.get("content", ""))
            )
            ...
            if content:
                await self.manager.broadcast(TaskThinkingMessage(task_id=task_id, content=content))
            ...
            await self.manager.broadcast(
                TaskThinkingMessage(task_id=task_id, content=f"Using tool: {tool_name}")
            )

        elif event_type == TaskEventType.TOOL_RESULT:
            result = event.data.get("result", "")
            await self.manager.broadcast(
                TaskThinkingMessage(task_id=task_id, content=f"Tool result: {str(result)[:200]}")
            )
            ...
            await self.manager.broadcast(TaskErrorMessage(task_id=task_id, error=error))

    async def handle_pause(self, message: TaskPauseMessage, websocket: WebSocket) -> None:
    ...
    async def handle_resume(self, message: TaskResumeMessage, websocket: WebSocket) -> None:
    ...
    async def handle_cancel(self, message: TaskCancelMessage, websocket: WebSocket) -> None:
    ...
    async def handle_correct(self, message: TaskCorrectMessage, websocket: WebSocket) -> None:
        ...
        if task:
            ...
            await self.manager.broadcast(
                TaskThinkingMessage(
                    task_id=message.task_id, content=f"User correction received: {message.message}"
                )
            )
...
class ChatHandler:
    ...
    def _register_handlers(self):
        ...
        self.manager.register_handler(MessageType.CHAT_MESSAGE, self.handle_message)

    async def handle_message(self, message: ChatMessage, websocket: WebSocket) -> None:
        ...
        self._conversations[conversation_id].append({"role": "user", "content": message.message})
        ...
        try:
                ...
                if event.type == TaskEventType.STREAMING:
                    ...
                    if content:
                        ...
                        await self.manager.send_to(
                            websocket,
                            ChatStreamMessage(
                                content=content, conversation_id=conversation_id, done=False
                            ),
                        )
                    ...
                    await self.manager.send_to(
                        websocket,
                        ChatStreamMessage(content="", conversation_id=conversation_id, done=True),
                    )
            ...
            await self.manager.send_to(
                websocket,
                ChatResponseMessage(
                    content=full_response, agent=message.agent, conversation_id=conversation_id
                ),
            )
            ...
            await self.manager.send_to(
                websocket,
                ChatResponseMessage(
                    content=f"Error: {str(e)}", agent="system", conversation_id=conversation_id
                ),
            )
...
class AgentStatusHandler:
    ...
    def _register_handlers(self):
        ...
        self.manager.register_handler(MessageType.AGENTS_STATUS, self.handle_status_request)

    async def handle_status_request(self, message: WSMessage, websocket: WebSocket) -> None:
        """Handle agent status request."""
        agents = self._orchestrator.list_tui_agents()
        await self.manager.send_to(websocket, AgentsStatusMessage(agents=agents))
...
class TAJINEHandler:
    ...
    async def _handle_tajine_event(self, task_id: str, event_data: dict[str, Any]) -> None:
        ...
        if phase == "perceive":
            ...
            ws_message = TAJINEThinkingMessage(
                task_id=task_id, session_id=session_id, content=message
            )
        else:
    ...
    def create_screenshot_callback(self, task_id: str, session_id: str | None = None):
        ...
        def callback(action: str, screenshot_b64: str, url: str | None = None):
            ...
            try:
                ...
                loop.create_task(
                    self._broadcast_screenshot(task_id, action, screenshot_b64, url, session_id)
                )
            except RuntimeError:
